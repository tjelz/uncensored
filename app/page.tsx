...
const FadeInWords = () => (
  <div className="mb-8 text-center select-none" aria-label="ASK ME ANYTHING. (LITERALLY)">
    {FADE_WORDS.map((word, idx) => (
      <span
        key={word}
        className={`block ${idx === 3 ? 'text-2xl md:text-3xl text-white/70' : 'text-5xl md:text-6xl'} font-extrabold tracking-tight uppercase`}
        style={{
          opacity: 0,
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col text-white font-bold">
      ...
      <header className="w-full flex justify-between items-center px-6 py-4 border-b border-white/10">
        <span className="text-2xl tracking-widest select-none">$uncensored</span>
        <div className="flex items-center gap-4">
          ...
          <button
            className="flex items-center gap-1 bg-white/10 text-white px-3 py-1 rounded cursor-not-allowed opacity-60 select-none"
          ...
          >
            ...
            <span className="font-mono">{PUMPFUN_CONTRACT}</span>
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" className="inline"><rect x="3" y="3" width="10" height="10" rx="2"/><path d="M7 7h2v2H7z"/></svg>
          </button>
        </div>
      </header>

      {/* Main Content */}
      <main className="flex flex-col items-center px-4 pt-8 pb-8">
        <FadeInWords />
        <section
          className="w-full max-w-lg bg-white/5 rounded-xl shadow-lg p-4 flex flex-col gap-4 border border-white/10"
          aria-label="Chatbox"
        >
          <div className="flex flex-col gap-4 max-h-[60vh] overflow-y-auto pr-1 text-xs md:text-sm" role="log" aria-live="polite">
            {messages.map((msg, idx) => (
              <div
                key={idx}
                className={
                  msg.sender === "user"
                    ? "self-end bg-white/20 text-white px-4 py-3 rounded-lg w-full text-right"
                    : "self-start bg-white/10 text-white/90 px-4 py-4 rounded-lg w-full prose prose-invert prose-pre:bg-black/80 prose-pre:text-xs prose-pre:rounded-lg"
                }
            ...
            {streamingMessage && (
              <div className="self-start bg-white/10 text-white/90 px-4 py-4 rounded-lg max-w-[80%] whitespace-pre-line prose prose-invert prose-pre:bg-black/80 prose-pre:text-xs prose-pre:rounded-lg" aria-live="polite">
          ...
          <div className="flex flex-col sm:flex-row items-end mt-2 w-full">
            <textarea
              ...
              className="flex-1 bg-black text-white px-4 py-2 focus:outline-none resize-none text-xs md:text-sm border border-white/20 rounded-lg placeholder-white/40 min-h-[40px] max-h-40"
            ...
            <button
              onClick={handleSend}
              className="ml-0 sm:ml-3 mt-2 sm:mt-0 px-6 py-2 bg-white text-black font-bold rounded-full shadow transition hover:shadow-lg hover:bg-white/90 focus:outline-none disabled:opacity-50 text-xs md:text-sm border-none"
